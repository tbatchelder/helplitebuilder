...
extension point); user-saveable themes (Save as Theme / My Themes,
independent of `customPresets` — see the decision log for why those
are two separate things); Import/Export; Settings/Pause; Save/Reset/View Site
...
- **Theme sharing.** Saved themes today are per-visitor, per-device,
  stored only locally — sharing one with someone else isn't currently
  supported. Deliberately deferred rather than built alongside Save as
  Theme; worth revisiting once the project has more real users, per an
  explicit decision (see the decision log).
